...
module.exports = function(lApp) {
	...
	router.get('/api/app/signout', function *() {
    ...
    router.post('/api/app/signin', function *(next) {
		var ctx = this;
		...
		yield passport.authenticate('local', function *(err, user, info) {
            ...
            user.language = ctx.request.body.language || 'en-US';

			// Store login information in session
			var m = yield Passport.appLogin(ctx, user);
    ...
    router.post('/api/app/signup', function *() {
		var username = this.request.body.username || null;
		var name = this.request.body.name || 'app user';
		var password = this.request.body.password || null;
		var email = this.request.body.email || null;
        var phone = this.request.body.phone || null;
		var language = this.request.body.language || 'en-US';
		...
		if (!password) {
			if (settings.general.features.requiredPassword) {
				restpack
					.setStatus(RestPack.Status.ValidationFailed)
					.appendError('password', RestPack.Code.Required);
			}
		...
		// Response token and basic data for app
		var m = yield Passport.appLogin(this, member);
